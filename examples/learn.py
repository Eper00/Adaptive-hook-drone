...
def adjust_difficulty(env, level,level_changed=True):
    if isinstance(env, FlyThroughAviary):
        env.TARGET_HEIGHT = [
    ...
    elif isinstance(env, AdaptiveHookHover):
        if level_changed:
...
def train_single(
    ...
    # -----------------------------
    if curriculum_flag:
        # 1) Single wrapped env for logging
        base_env = env_class(ctrl_freq=48, sim_freq=240)
        wrapped_env = CurriculumWrapper(base_env, difficulty_fn=adjust_difficulty)
        ...
        wrapped_env = None  # no curriculum stats
    ...
    # -----------------------------
    if curriculum_flag:
        print("\n=== Curriculum Level-Up Events ===")
        print(wrapped_env.level_up_times)
