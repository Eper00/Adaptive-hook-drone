...
class AdaptiveTransportDirectorAviary(AdaptiveTransportAviary):
    ...
    def step(self, action):
        ...
        obs = self._get_low_level_obs(target_vel)
        low_level_action, _ = self.controller_model.predict(obs, deterministic=True)
        if not self.GRAB_FLAG_ENABLE:
            low_level_action[4:] = 0
            self.GRAB_FLAG = False
            ...
            if self.GRAB_FLAG and self.current_waypoint_idx[0] == pickup_idx:
                payload_pos = self.data.qpos[
                    self.target_qpos_adr:self.target_qpos_adr + 3
                ]
                hook_pos = self.data.xpos[self.segment_2_id]

                if payload_pos[1] < hook_pos[1]:
                    low_level_action[4] = 1
                    low_level_action[5] = -1
                    self.tendon_orientation = 1
                else:
                    low_level_action[4] = -1
                    low_level_action[5] = 1
            ...
            # Payload elengedése után a hook maradjon ugyanabban az irányban
    ...
    def _waypoint_reward(self, wp_idx):
        if wp_idx == 0:
            return 20.0
        if wp_idx == 1:
            return 20.0
